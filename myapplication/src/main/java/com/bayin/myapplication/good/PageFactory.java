package com.bayin.myapplication.good;
...
import com.bayin.myapplication.good.db.BookCatalogue;
import com.bayin.myapplication.good.db.BookList;
import com.bayin.myapplication.good.util.BitmapUtil;
import com.bayin.myapplication.good.util.BookUtil;
import com.bayin.myapplication.good.util.CommonUtil;
import com.bayin.myapplication.good.util.FileUtils;
import com.bayin.myapplication.good.util.TRPage;
...
//        mHeight = metric.heightPixels;
        ...
        mVisibleWidth = mWidth - marginWidth * 3;
...
//                    mContext.getResources(), R.drawable.main_bg, mWidth, mHeight));
    ...
    private void calculateLineCount(){
        mLineCount = (int) (mVisibleHeight / (m_fontSize + lineSpace))*2;// 可显示的行数
    }
...
//        word.setLength(0);
        ...
        if (m_lines.size() > 0) {
            float y_left = marginHeight;
            float y_right = marginHeight;
            for (int i = 0; i < m_lines.size(); i++) {
                if (i < mLineCount / 2) {
                    String strLine = m_lines.get(i);
                    y_left += (m_fontSize + lineSpace);
                    //从（x,y）坐标将文字绘于手机屏幕
                    c.drawText(strLine, 0, strLine.length(), marginWidth, y_left, mPaint);
                } else {
                    String strLine = m_lines.get(i);
                    y_right += (m_fontSize + lineSpace);
                    //从（x,y）坐标将文字绘于手机屏幕
        ...
        switch (type){
            case Config.BOOK_BG_DEFAULT:
                ...
                bitmap = BitmapUtil.decodeSampledBitmapFromResource(
                        mContext.getResources(), R.mipmap.book_bg_green, mWidth, mHeight);
